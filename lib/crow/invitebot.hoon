...
++  seed
  ...
  ::
    ...
    ?.  |(=(our.bol author.post.sadle) (~(has in boss) author.post.sadle))
      `[%keep me !>(`(unit (set ship))``boss)]
    ::
    ?:  ?=([[%text @] ~] contents.post.sadle)
      ?~  comm=(slice +.i.contents.post.sadle)
        `[%keep me !>(`(unit (set ship))``boss)]
      ?.  =((sy ~['!invitebot' '%list']) (sy u.comm))
        `[%keep me !>(`(unit (set ship))``boss)]
      =-  `[%says me [%text -]~ res !>(`(unit (set ship))``boss)]
      %-  ~(rep in boss)
      |=([a=@p b=@t] (rap 3 ~[(scot %p a) '\0a' b]))
    ::
    ?.  ?=([[%text @] [%mention @] * ~] contents.post.sadle)
      `[%keep me !>(`(unit (set ship))``boss)]
    ?~  comm=(slice +.i.contents.post.sadle)
      `[%keep me !>(`(unit (set ship))``boss)]
    ::
    ...
    ?:  (~(has in coom) '%delegate')
      =.  boss  (~(put in boss) +.i.t.contents.post.sadle)
      `[%keep me !>(`(unit (set ship))``boss)]
    ::
    ?:  (~(has in coom) '%demote')
      =.  boss  (~(del in boss) +.i.t.contents.post.sadle)
      `[%keep me !>(`(unit (set ship))``boss)]
    ::
    ?:  (~(has in coom) '%invite')
      =+  who=+.i.t.contents.post.sadle
      (hello mind res post.sadle who)
    ::
    `[%keep me !>(`(unit (set ship))``boss)]
    ++  slice
      |=  curd=cord
    ...
    ++  hello
      ...
      ?~  gorp
        =/  msg=(list content:store)
          :~  [%mention author.post]
              ...
              [%reference [%group res]]
          ==
        `[%says me msg res !>(`(unit (set ship))`mind)]
      =/  inv=action:invite
      ...
      =/  msg=(list content:store)
        :~  [%mention author.post]  [%text 'I\'ve invited ']
            [%mention who]
        ==
      :_  [%says me msg res !>(`(unit (set ship))`mind)]
